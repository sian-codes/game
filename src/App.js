...
class App extends Component {
  ...
  componentDidMount() {
    this.periodicInterval = setInterval( () =>{
      ...
      if (!this.state.isPause) {
        ...
        // TODO change the score to points system
        // score is created by an interval on the state
 ...
 buttonPress() {
  ...
  shiftRight = (isRight) => {
    ...
    //Making sure we are not overlapping other shape
    let isConflict = false;

    curShape.forEach(oldArray =>  {
      // Removing elements that are not part of block
    ...
    // Shifting if there is no conflict
    if (!isConflict) {
  ...
  rotateClockwise = (isClockwise) => {
    ...
    newShape.forEach(newArray => {
      // changing pos for element which are present in pos i.e it is not equal to default
  ...
  getNextBlock = () => {
    ...
    for(let i=0; i< curShape.length; i++) {
      ...
      if (isFilled) {
        ...
        // dropping the above row by one column
  ...
  shiftDown = () => {
    ...
    curShape[0].forEach((_, pos) => {
      ...
      let newArray = curShape.map(row => (row[pos]=== s.DEFAULT_VALUE)? -3: row[pos] + ROW_SIZE)
      ...
      if (
        ...
        if (this.state.yPos <= 0 && this.state.yPos !== -1){
          this.getNextBlock();
          alert('Game Over');
          this.resetGame();
        } else {
  ...
  render() {
    ...
    return (
        //TODO Create buttons & amend UX/UI
      <div className="App">
        <h1>  Tetris  {this.state.score}</h1>
        <p>  Right|Left|Down|Z|X|Space </p>
        <div style={style}> {board} </div>
          <div className="buttons">
        <button onClick={this.buttonPress}>Right</button>
        <button onClick={this.buttonPress}>Left</button>
        <button onClick={this.buttonPress}>Clockwise</button>
        <button onClick={this.buttonPress}>Anti-Clockwise</button>
          </div>
      </div>
    );
  }
